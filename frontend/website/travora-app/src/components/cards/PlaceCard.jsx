...
export default function PlaceCard({
  ...
  return (
    <div
      onClick={onPress}
      className={`w-full bg-white rounded-[18px] shadow-md border border-gray-100 hover:shadow-xl overflow-hidden cursor-pointer transition-transform duration-300 hover:-translate-y-1 ${className || ''}`}
    >
      <div className="h-[170px] w-full overflow-hidden relative">
        <img src={imageSrc} alt={title} className="w-full h-full object-cover" />
        {tag && (
          <span className="absolute top-3 left-3 text-[11px] font-semibold text-[#2f5aa5] bg-[#E9F2FF] px-3 py-1 rounded-full shadow-sm">
            {tag}
          </span>
        )}
        <div className="absolute top-3 right-3 bg-white/95 text-gray-800 text-xs font-semibold rounded-full px-3 py-1 shadow-sm flex items-center gap-1">
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        <div className="mt-2 flex justify-end">
          {priceLabel ? (
            ...
            <button
              className="text-[11px] font-bold text-white bg-[#82B1FF] px-5 py-2 rounded-md shadow-sm hover:bg-[#6aa5ff] transition"
              onClick={(e) => {
                e.stopPropagation();
                onPress?.();
              }}

...
import 'profile.dart';
import 'destination.dart'; // Pastikan file destination.dart ada
...
// --- 1. NAVIGATION BAR (FIXED CENTER WITH STACK) ---
  ...
  void _handleTabSelection(int index) {
    if (index == 0) { 
      ...
      if (ModalRoute.of(context)?.settings.name != '/' && widget.selectedIndex != 0) {
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const HomePage())).then((_) => _scrollToEvent());
      } else {
        _scrollToEvent();
      }
    } else if (index == 2) { 
...
class SearchResultsSection extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            query.isEmpty ? "Search Result" : query,
            style: GoogleFonts.poppins(
              fontSize: 36,
              fontWeight: FontWeight.bold,
              color: Colors.black,
  ...
  Widget _buildResultCard(BuildContext context, {required String imageUrl, required String title}) {
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Protected forest with free-roaming monkeys.',
                    style: GoogleFonts.poppins(
                      fontSize: 12,
                      color: Colors.grey,
                    ),
                  ...
                  Align(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Text('LEARN MORE', style: TextStyle(fontSize: 10, color: Colors.white)),
                    ),
                  )
                ],
              ),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(flex: 4, child: Row(children: [InkWell(onTap: () => _scroll(-240), borderRadius: BorderRadius.circular(50), child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.blue.withOpacity(0.3), width: 2), color: Colors.white), child: const Icon(Icons.arrow_back, color: Colors.blue))), const SizedBox(width: 15), Expanded(child: SizedBox(height: 320, child: ListView(controller: _scrollController, scrollDirection: Axis.horizontal, clipBehavior: Clip.hardEdge, children: [_buildPlaceCard(context, imageUrl: 'https://images.unsplash.com/photo-1544644181-1484b3fdfc62?q=80&w=600&auto=format&fit=crop', title: 'Monkey Forest', subtitle: 'Protected forest with\nfree-roaming\nmonkeys.', rating: '4.9'), const SizedBox(width: 20), _buildPlaceCard(context, imageUrl: 'https://images.unsplash.com/photo-1555400038-63f5ba517a47?q=80&w=600&auto=format&fit=crop', title: 'Ulun Danu Beratan', subtitle: 'Iconic temple on the\nshores of Lake\nBeratan.', rating: '4.9'), const SizedBox(width: 20), _buildPlaceCard(context, imageUrl: 'https://images.unsplash.com/photo-1518548419970-58e3b4079ab2?q=80&w=600&auto=format&fit=crop', title: 'Campuhan Ridge', subtitle: 'Scenic nature trek\nwith lush valley\nviews.', rating: '4.8'), const SizedBox(width: 20), _buildPlaceCard(context, imageUrl: 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=600&auto=format&fit=crop', title: 'Tanah Lot', subtitle: 'Ancient Hindu shrine\nperched on top of\nan outcrop.', rating: '4.7'), const SizedBox(width: 20), _buildPlaceCard(context, imageUrl: 'https://images.unsplash.com/photo-1577717903315-1691ae25ab3f?q=80&w=600&auto=format&fit=crop', title: 'Tegalalang Rice', subtitle: 'Famous rice terraces\nusing subak irrigation.', rating: '4.9')]) )), const SizedBox(width: 15), InkWell(onTap: () => _scroll(240), borderRadius: BorderRadius.circular(50), child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.blue.withOpacity(0.3), width: 2), color: Colors.white), child: const Icon(Icons.arrow_forward, color: Colors.blue)))]))
        ],
      ),
    );
  }

  Widget _buildPlaceCard(BuildContext context, {required String imageUrl, required String title, required String subtitle, required String rating}) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
...
class ExploreUbudSection extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 40),
          SizedBox(
            height: 400, // Tinggi area grid
            child: Row(
              children: [
                Expanded(
                  child: _buildClickableImage(context, 'https://images.unsplash.com/photo-1518548419970-58e3b4079ab2?q=80&w=800&auto=format&fit=crop'),
                ),
                const SizedBox(width: 20),
                Expanded(
                  child: Column(
                    children: [
                      Expanded(
                        child: _buildClickableImage(context, 'https://images.unsplash.com/photo-1554481923-a6918bd997bc?q=80&w=800&auto=format&fit=crop'),
                      ),
                      const SizedBox(height: 20),
                      Expanded(
                        child: _buildClickableImage(context, 'https://images.unsplash.com/photo-1554481923-a6918bd997bc?q=80&w=800&auto=format&fit=crop'),
                ...
                const SizedBox(width: 20),
                Expanded(
                  child: _buildClickableImage(context, 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=800&auto=format&fit=crop'),
                ),
              ],
            ),
          )
  ...
  // Helper untuk membuat gambar bisa diklik
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 40),
          Row(children: [Expanded(child: _buildStyleCard(context, imageUrl: 'https://images.unsplash.com/photo-1518548419970-58e3b4079ab2?q=80&w=400', title: 'Monkey Forest', description: 'Protected forest with free-roaming monkeys.')), const SizedBox(width: 20), Expanded(child: _buildStyleCard(context, imageUrl: 'https://images.unsplash.com/photo-1537953773345-d172ccf13cf1?q=80&w=400', title: 'Sacred Monkey', description: 'Ancient temple complex in Ubud.')), const SizedBox(width: 20), Expanded(child: _buildStyleCard(context, imageUrl: 'https://images.unsplash.com/photo-1598091383021-15ddea10925d?q=80&w=400', title: 'Ubud Yoga', description: 'Find inner peace in the heart of Bali.')), const SizedBox(width: 20), Expanded(child: _buildStyleCard(context, imageUrl: 'https://images.unsplash.com/photo-1516483638261-f4dbaf036963?q=80&w=400', title: 'Local Culture', description: 'Experience the daily life of locals.'))]),
          const SizedBox(height: 40),
...
class TopRatedSection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(color: const Color(0xFFF2F7FB), padding: const EdgeInsets.symmetric(horizontal: 150.0, vertical: 40.0), child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: [Container(width: 4, height: 28, color: Colors.black), const SizedBox(width: 10), Text('Top-Rated Travel Experiences', style: GoogleFonts.poppins(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.black))]), Row(children: [Text('All', style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black, decoration: TextDecoration.underline)), const SizedBox(width: 20), Text('Adventure', style: GoogleFonts.poppins(fontSize: 18, color: Colors.black87)), const SizedBox(width: 20), Text('Culture', style: GoogleFonts.poppins(fontSize: 18, color: Colors.black87))])]), const SizedBox(height: 40), Row(children: [Expanded(child: _buildTopRatedCard(context, imageUrl: 'https://images.unsplash.com/photo-1544644181-1484b3fdfc62?q=80&w=600&auto=format&fit=crop', title: 'Monkey Forest Ubud', description: 'Protected forest.')), const SizedBox(width: 20), Expanded(child: _buildTopRatedCard(context, imageUrl: 'https://images.unsplash.com/photo-1555400038-63f5ba517a47?q=80&w=600&auto=format&fit=crop', title: 'Ulun Danu', description: 'Iconic temple.')), const SizedBox(width: 20), Expanded(child: _buildTopRatedCard(context, imageUrl: 'https://images.unsplash.com/photo-1518548419970-58e3b4079ab2?q=80&w=600&auto=format&fit=crop', title: 'Campuhan', description: 'Scenic nature trek.')), const SizedBox(width: 20), Expanded(child: _buildTopRatedCard(context, imageUrl: 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=600&auto=format&fit=crop', title: 'Tanah Lot', description: 'Ancient Hindu shrine.'))]), const SizedBox(height: 40), Row(mainAxisAlignment: MainAxisAlignment.center, children: [IconButton(onPressed: () {}, icon: const Icon(Icons.arrow_circle_left_outlined, color: Color(0xFF82B1FF), size: 40)), const SizedBox(width: 10), Text('Back', style: GoogleFonts.poppins(fontSize: 18, color: Colors.grey[700])), const SizedBox(width: 20), Text('Next', style: GoogleFonts.poppins(fontSize: 18, color: Colors.black)), const SizedBox(width: 10), IconButton(onPressed: () {}, icon: const Icon(Icons.arrow_circle_right_outlined, color: Color(0xFF82B1FF), size: 40))])]));
  }
  Widget _buildTopRatedCard(BuildContext context, {required String imageUrl, required String title, required String description}) { return GestureDetector(onTap: () {Navigator.push(context, MaterialPageRoute(builder: (context) => const DestinationPage()));}, child: Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.1), spreadRadius: 2, blurRadius: 8, offset: const Offset(0, 2))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(16)), child: Image.network(imageUrl, height: 180, width: double.infinity, fit: BoxFit.cover)), Padding(padding: const EdgeInsets.all(16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: const [Icon(Icons.star, color: Colors.amber, size: 16), SizedBox(width: 4), Text('4.9', style: TextStyle(fontWeight: FontWeight.bold))]), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.blue[50], borderRadius: BorderRadius.circular(12)), child: const Text('Adventure', style: TextStyle(fontSize: 10, color: Colors.blue, fontWeight: FontWeight.bold)))]), const SizedBox(height: 12), Text(title, style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis), const SizedBox(height: 6), Text(description, style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey), maxLines: 2, overflow: TextOverflow.ellipsis), const SizedBox(height: 16), Align(alignment: Alignment.centerRight, child: ElevatedButton(onPressed: () {Navigator.push(context, MaterialPageRoute(builder: (context) => const DestinationPage()));}, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF82B1FF), elevation: 0, minimumSize: const Size(80, 32), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6))), child: const Text('LEARN MORE', style: TextStyle(fontSize: 10, color: Colors.white))))]))]))); }
}

// --- 7. TODAYS EVENT SECTION ---
...
class _TodaysEventSectionState extends State<TodaysEventSection> {
  ...
  Widget _buildEventCard(BuildContext context, Map<String, String> event) { 
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.grey.withOpacity(0.1)), boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.05), spreadRadius: 2, blurRadius: 10, offset: const Offset(0, 4))]), 
...
// --- 8. TRAVORA CHAT SECTION (UNCHANGED) ---
          ...
          Expanded(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                Text(
                  "Meet Travora’s smart WhatsApp assistant — your travel buddy for instant answers and recommendations.",
                  style: GoogleFonts.poppins(
                ...
                ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF64829F),
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          _buildChatBubble(
                              'Halo! Saya Asisten Chat. Mau cari apa hari ini?',
                              isUser: false),
                          _buildChatBubble('Wisata yang terpopuler di ubud',
                              isUser: true),
                          _buildChatBubble(
                              'Berikut 5 wisata yang terpopuler : Monkey Forest........',
                              isUser: false),
                          _buildChatBubble('Tolong arah ke wisata Monkey',
                              isUser: true),
                          _buildChatBubble('Mau buka Google Maps?',
                              isUser: false),
                        ],
                      ),
                    ),
...
// --- 9. FOOTER SECTION (UNCHANGED) ---
class FooterSection extends StatelessWidget {
  final VoidCallback onScrollToTop;
  const FooterSection({Key? key, required this.onScrollToTop}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Container(color: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 150.0, vertical: 50.0), child: Column(children: [Stack(alignment: Alignment.center, children: [Row(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.location_on_outlined, color: Color(0xFF5E9BF5), size: 36), const SizedBox(width: 8), Text('Travora', style: GoogleFonts.poppins(fontSize: 32, fontWeight: FontWeight.bold, color: const Color(0xFF5E9BF5)))]), Align(alignment: Alignment.centerRight, child: InkWell(onTap: onScrollToTop, borderRadius: BorderRadius.circular(30), child: Container(width: 50, height: 50, decoration: const BoxDecoration(color: Color(0xFF82B1FF), shape: BoxShape.circle), child: const Icon(Icons.keyboard_arrow_up, color: Colors.white, size: 30))))]), const SizedBox(height: 40), Divider(color: Colors.grey[300], thickness: 1), const SizedBox(height: 20), Align(alignment: Alignment.centerRight, child: Text('Copyright © 2025 • Travora.', style: GoogleFonts.poppins(fontSize: 14, color: Colors.black87)))]));
  }
}

// --- LOGIN & SIGNUP PAGES ---
// (Includes CustomLoginShapeClipper)
...
class SignUpPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Colors.white, body: Row(children: [Expanded(flex: 4, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 80.0), child: Center(child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Row(children: [const Icon(Icons.location_on_outlined, color: Color(0xFF5E9BF5), size: 40), const SizedBox(width: 8), Text('Travora', style: GoogleFonts.poppins(fontSize: 32, fontWeight: FontWeight.bold, color: const Color(0xFF5E9BF5)))]), const SizedBox(height: 30), Text('Sign up', style: GoogleFonts.poppins(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.black)), const SizedBox(height: 30), _buildTextField(label: 'Name'), const SizedBox(height: 20), _buildTextField(label: 'Email'), const SizedBox(height: 20), _buildTextField(label: 'Password', isPassword: true), const SizedBox(height: 20), _buildTextField(label: 'Confirm Password', isPassword: true), const SizedBox(height: 30), SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: () { Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const LoginPage())); }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF82B1FF), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)), elevation: 0), child: Text('Sign up', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)))), const SizedBox(height: 20), Row(mainAxisAlignment: MainAxisAlignment.center, children: [Text("Already have a UBX account? ", style: GoogleFonts.poppins(color: Colors.black)), InkWell(onTap: () { Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const LoginPage())); }, child: Text("Login", style: GoogleFonts.poppins(color: const Color(0xFF82B1FF), fontWeight: FontWeight.bold)))],)]))))), Expanded(flex: 6, child: ClipPath(clipper: CustomLoginShapeClipper(), child: Container(height: double.infinity, decoration: const BoxDecoration(image: DecorationImage(image: NetworkImage('https://images.unsplash.com/photo-1537996194471-e657df975ab4?auto=format&fit=crop&q=80'), fit: BoxFit.cover)), child: Container(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.centerLeft, end: Alignment.centerRight, colors: [Colors.black.withOpacity(0.3), Colors.transparent])), child: Center(child: Padding(padding: const EdgeInsets.only(left: 100.0, right: 50.0), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.public, color: Colors.white, size: 40), const SizedBox(width: 15), Text("Let's discover the beauty of Bali", style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white))],)))))))]));
  }
  Widget _buildTextField({required String label, bool isPassword = false}) { return Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 5), decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(8)), child: TextField(obscureText: isPassword, decoration: InputDecoration(border: InputBorder.none, labelText: label, labelStyle: GoogleFonts.poppins(color: Colors.grey), contentPadding: EdgeInsets.zero))); }
}
